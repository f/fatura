  ...
  return (
    <aside className="section-block docs-sidebar">
      ...
      <nav style={{ marginTop: "0.7rem", display: "grid", gap: "0.8rem" }}>
        <Link href="/dokumantasyon" className={`docs-link ${typeof activeSlug === "undefined" ? "is-active" : ""}`}>
          Genel bakış
        </Link>

        {Object.entries(groupedDocs).map(([group, docs]) => (
          <section key={group}>
            <p className="text-muted" style={{ fontSize: "0.76rem", letterSpacing: "0.08em", marginBottom: "0.4rem" }}>
              {group.toUpperCase()}
            </p>
      ...
      >
        <p
          ...
          style={{ fontSize: "0.76rem", letterSpacing: "0.06em" }}
        >
          DÜZ METİN (LLM)
        </p>
