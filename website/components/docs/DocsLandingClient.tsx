...
function filterGroupedDocs(
	...
	const filtered =
		normalized.length === 0
			? docs
			: docs.filter((doc) => {
					const haystack = [doc.title, doc.description, doc.group]
						.filter(Boolean)
						.join(" ")
						.toLowerCase();
...
export function DocsLandingClient({
	docs,
	groupedDocs,
	...
	return (
		<div className="docs-layout">
			<DocsSidebar
				groupedDocs={
					query.trim().length > 0 ? filteredGroups : groupedDocs
				}
			/>
			...
			>
				<section className="section-block">
					...
					>
						Kurulumdan API kullanımına: adım adım rehber ve
						örnekler.
					</h1>
					<p className="text-muted" style={{ marginTop: "0.8rem" }}>
						Aşağıdaki listeden konu seçebilir veya arama kutusuyla
						başlık ve açıklamalarda arama yapabilirsiniz.
					</p>
					<p
						className="text-muted"
						style={{ marginTop: "0.55rem", fontSize: "0.88rem" }}
					>
