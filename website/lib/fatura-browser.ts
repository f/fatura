...
export class BrowserFaturaClient {
    ...
    window.fetch = (async (
      ...
      const body =
        typeof bodySource === "string"
          ? bodySource
          : bodyToString(bodySource as BodyInit | undefined);
  ...
  async getAllInvoicesIssuedToMeByDateRange(
    ...
    return this.runWithProxy(() =>
      this.client.getAllInvoicesIssuedToMeByDateRange(token, range)
    );
  }
  ...
  async createInvoiceAndGetHTML(
    ...
    return this.runWithProxy(() =>
      this.client.createInvoiceAndGetHTML(userId, password, invoiceDetails, options)
    );
  }
}
