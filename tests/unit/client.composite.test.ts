...
import { minimalInvoice, draftInvoiceResponse, invoiceListItem } from "../fixtures/invoice.fixture";
...
describe("FaturaClient — composite (high-level) methods", () => {
    ...
    describe("createInvoice", () => {
        ...
        it("calls methods in the correct order (getToken → createDraft → find → sign)", async () => {
            ...
            await client.createInvoice(USER_ID, PASSWORD, minimalInvoice);
            expect(callOrder).toEqual(["getToken", "createDraftInvoice", "findInvoice", "signDraftInvoice"]);
        });
    });
    ...
    describe("createInvoiceAndGetDownloadURL", () => {
        it("returns a URL string", async () => {
            ...
            const result = await client.createInvoiceAndGetDownloadURL(USER_ID, PASSWORD, minimalInvoice);
        ...
        it("passes options through to createInvoice", async () => {
            ...
            expect(createInvoiceSpy).toHaveBeenCalledWith(USER_ID, PASSWORD, minimalInvoice, {
                sign: false,
            });
        });
    ...
    describe("createInvoiceAndGetHTML", () => {
        it("returns an HTML string", async () => {
            ...
            const result = await client.createInvoiceAndGetHTML(USER_ID, PASSWORD, minimalInvoice);
        ...
        it("calls getInvoiceHTML with token, uuid, and { signed } from createInvoice", async () => {
            ...
            const getHTMLSpy = vi.spyOn(client, "getInvoiceHTML").mockResolvedValue("<html></html>");
