...
async function getTokenWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const isLocked =
        lastError.message.includes("birden fazla giriş") ||
        lastError.message.includes("Güvenli Çıkış");
  ...
  describe("getAllInvoicesIssuedToMeByDateRange", () => {
    it("returns an array or throws a GİB permission error", async () => {
      ...
      try {
        ...
        console.warn(`  ↳ getAllInvoicesIssuedToMeByDateRange: ${(err as Error).message} (hesap yetersiz — beklenen)`);
      }
    });
  });
    ...
    it("signs and cancels when account has permission; skips gracefully otherwise", async () => {
      ...
      try {
        ...
        const isPermissionError =
          msg.includes("yetkiniz yok") || msg.includes("yetki") || msg.includes("HSM");
