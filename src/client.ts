...
export class FaturaClient {
  ...
  async createDraftInvoice(
    ...
    const invoice = await this.runCommand<ApiResponse>(
      token,
  ...
  ): Promise<InvoiceListItem | undefined> {
  ...
  async cancelDraftInvoice(
    ...
    const result = await this.runCommand<ApiResponse>(
      token,
      ...COMMANDS.cancelDraftInvoice,
  ...
  async getAllInvoicesByDateRange(
    token: string,
  ...
  async getInvoiceHTML(
    ...
    const result = await this.runCommand<ApiResponse<string>>(
      token,
      ...COMMANDS.getInvoiceHTML,
      { ettn: uuid, onayDurumu: signed ? "Onaylandı" : "Onaylanmadı" }
    );
    return result.data;
  }
  ...
  async getUserData(token: string): Promise<UserData> {
    const result = await this.runCommand<ApiResponse<RawUserData>>(
      token,
  ...
  async getRecipientDataByTaxIDOrTRID(
    token: string,
    taxIDOrTRID: string
  ): Promise<unknown> {
    const result = await this.runCommand<ApiResponse>(
  ...
  async sendSignSMSCode(
    token: string,
    phone: string
  ): Promise<string | undefined> {
    const result = await this.runCommand<SmsResponse>(
      token,
      ...COMMANDS.sendSignSMSCode,
  ...
  async verifySignSMSCode(
    ...
    const result = await this.runCommand<SmsResponse>(
      token,
      ...COMMANDS.verifySMSCode,
      { SIFRE: smsCode, OID: operationId }
    );
    return result.oid;
  }
  ...
  async createInvoiceAndGetDownloadURL(
    ...
    const { token, uuid, signed } = await this.createInvoice(
      userId,
      password,
      invoiceDetails,
      options
